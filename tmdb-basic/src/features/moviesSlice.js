...
// 영화 상세 정보 가져오기
export const fetchMovieDetails = createAsyncThunk('movies/fetMovieDetails', async (movieId) => {
...
const moviesSlice = createSlice({
   ...
   reducers: {},
   // extraReducers: 비동기 액션이 발생했을떄 state를 바꿔줌
   extraReducers: (builder) => {
      builder
         // 데이터를 가져오는 동안
         // fetchMovies(): 액션 생성자 함수 열할
         .addCase(fetchMovies.pending, (state) => {
            state.loading = true
            state.error = null // 다른 액션 생성자 함수에서 에러 발생 시 에러 메세지가 남아있는 경우를 대비해 초기화
         })
         // 데이터를 성공적으로 가져온 경우
         //
...
export default moviesSlice.reducer // 리듀서 함수 내보냄
