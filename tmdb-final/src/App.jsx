...
function App() {
   return (
      <Routes>
         ...
         문제는 page state값이 초기화 되지 않고 유지된다. 그러면 새로운 메뉴를 클릭했을때 해당 페이지 번호가 1이 되지 못하므로 문제가 발생한다.
         => 따라서 기존 MovieCategory를 언마운트 하고 새로 마운트가 일어나도록 하기 위해서 key props를 사용(page state가 모두 1로 초기화된다.)

         category props 상관없이 key props만 지정해도 언마운트 후 마운트됨
         key는 라이터가 컴포넌트를 구분하기 위한 props
